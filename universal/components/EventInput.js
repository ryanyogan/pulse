...
export default class EventInput extends Component {
  ...
  render() {
    const saveText = (this.props.editing) ? 'Save' : 'Add';
    const className = Object.keys(VALUE_CLASSES).reduce((current, key) => {
      ...
      <form className='Pulse-eventInput pure-form'>
        <fieldset>
          ...
          <input className={className}
                 type='range' id='value' min='1' max='100'
                 value={this.state.value}
                 onChange={::this.handleValueChange} />
          <span className='Pulse-eventInput-value'>{this.state.value}</span>
          <button type='submit' className='save pure-render'
